...
function convertPasswordToSha1(userName) {
    ...
    try {
        ...
        console.error("convertPasswordToSha1",e.message);
        return e.message;
    }
};
...
function hashGenerator(userName, senderId, message, secureKey) {
    ...
    try {
        ...
        console.error("hashGenerator", e.message);
        return e.message;
    }
    return digestHash;
};
...
export class SMSServices {
    ...
    // send sendSingleSMS to user
    async sendSingleSMSForBulk(userName, password, senderId, message, mobileno, secureKey, templateId) {
        try {
            let data = {
                username: userName.trim(),
                password: convertPasswordToSha1(password),
            ...
            Logger.error("sendSingleSMSForBulk",e);
            return e.message;
        }
    };
